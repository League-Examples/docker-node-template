...
export class EmailService {
  // PrismaClient is accepted for constructor consistency even if unused here
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  constructor(_prisma: any) {}

  /** Send the official monthly review email to a guardian. */
  async sendReviewEmail(params: ReviewEmailParams): Promise<EmailResult> {
  ...
  /** Send a test (preview) review email without marking the review as sent. */
  async sendTestReviewEmail(params: ReviewEmailParams): Promise<EmailResult> {
    if (!process.env.SENDGRID_API_KEY) {
      console.warn('[EmailService] SENDGRID_API_KEY not set — skipping test email send');
      return { success: false, reason: 'SENDGRID_API_KEY not configured' };
    }
